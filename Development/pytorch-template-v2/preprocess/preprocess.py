...
class TransformerPreprocess:
    ...
    def __preprocessing(self, df, all_df):
    ...
    def __feature_engineering(self, df, all_df):
        
        # 시험지 대분류
        def get_large_paper_number(x):
            return x[1:4]
        df['large_paper_number'] = df['assessmentItemID'].apply(lambda x : get_large_paper_number(x))
        all_df['large_paper_number'] = all_df['assessmentItemID'].apply(lambda x : get_large_paper_number(x))
        ...
        all_df = get_now_elapsed(df = all_df)

        # 문항별 정답률
        df = df.set_index('assessmentItemID')
        df['assessmentItemID_mean_answerCode'] = all_df[all_df['answerCode'] != -1].groupby('assessmentItemID').mean()['answerCode']
        df = df.reset_index(drop = False)

        # 문항별 정답률 표준편차
        df = df.set_index('assessmentItemID')
        df['assessmentItemID_std_answerCode'] = all_df[all_df['answerCode'] != -1].groupby('assessmentItemID').std()['answerCode']
        df = df.reset_index(drop = False)

        # 올바르게 푼 사람들의 문항별 풀이 시간 평균
        df = df.set_index('assessmentItemID')
        df['assessmentItemID_mean_now_elapsed'] = all_df[all_df['answerCode'] == 1].groupby('assessmentItemID').mean()['now_elapsed']
        df = df.reset_index(drop = False)

        # 올바르게 푼 사람들의 문항별 풀이 시간 표준 편차
        df = df.set_index('assessmentItemID')
        df['assessmentItemID_std_now_elapsed'] = all_df[all_df['answerCode'] == 1].groupby('assessmentItemID').std()['now_elapsed']
        df = df.reset_index(drop = False)
    ...
    def load_data_from_file(self, data_dir, is_train=True):
        ...
        if is_train:
            ...
            df = pd.concat([train_df, test_df[test_df['answerCode'] != -1]]).reset_index(drop=True)
        ...
        df, all_df = self.__feature_engineering(df = df, all_df = all_df)
        df, all_df = self.__preprocessing(df = df, all_df = all_df)
