...
class DecoderMultiHeadAttention(nn.Module):
    ...
    def forward(self, input_enc, input_dec, mask):
        ...
        """
        residual = input_dec # residual connection을 위해 residual 부분을 저장
        batch_size, seqlen = input_dec.size(0), input_dec.size(1)

        # Query, Key, Value를 (num_head)개의 Head로 나누어 각기 다른 Linear projection을 통과시킴
        Q = self.W_Q(input_dec).view(batch_size, seqlen, self.num_heads, self.hidden_units // self.num_heads) # (batch_size, max_len, num_heads, hidden_units)
        K = self.W_K(input_enc).view(batch_size, seqlen, self.num_heads, self.hidden_units // self.num_heads) # (batch_size, max_len, num_heads, hidden_units)
        V = self.W_V(input_enc).view(batch_size, seqlen, self.num_heads, self.hidden_units // self.num_heads) # (batch_size, max_len, num_heads, hidden_units)
